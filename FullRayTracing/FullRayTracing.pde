...
void draw()
{
  ...
  //PointLightSource pls1 = new PointLightSource(new PVector(-5, 5, -5), new FColor(1, 1, 1));
  //lightSources.add(pls1);
  //PointLightSource pls2 = new PointLightSource(new PVector(-20, 5, -5), new FColor(1, 1, 1));
  //lightSources.add(pls2);
  //  PointLightSource pls3 = new PointLightSource(new PVector(-20, 5, -5), new FColor(1, 1, 1));
  //lightSources.add(pls3);
      PointLightSource pls4 = new PointLightSource(new PVector(1000, 5, -5), new FColor(1, 1, 1));
  scene.lightSources.add(pls4);
      PointLightSource pls5 = new PointLightSource(new PVector(-1110, 5, -5), new FColor(1, 1, 1));
  scene.lightSources.add(pls5);
        PointLightSource pls6 = new PointLightSource(new PVector(-1110, 511, -2225), new FColor(1, 1111, 1));
  scene.lightSources.add(pls6);
  ...
  for (int y = 0; y <height; ++ y)
  {
    ...
    for (int x = 0; x <width; ++x)
    {
      ...
      IntersectionTestResult res = scene.testIntersectionWithAll(eyeRay);
      
      if(res.shape != null)
      {
        kAmb =  res.shape.material.ambientFactor;
        kDif =  res.shape.material.diffuseFactor;
        kSpe =  res.shape.material.specularFactor;
        shine = res.shape.material.shininess;
      }
      if (res != null)
      {
       FColor col = new FColor(0, 0, 0);
        ...
        {
        IntersectionPoint res_true = res.intersectionPoint;
        Lighting ltg = scene.lightSources.get(l).lightingAt(res_true.position);
        float nlDot = constrain(res_true.normal.dot(ltg.direction), 0, 1);
        col.add(new FColor (255*constrain(kDif.red*nlDot, 0, 1), 255*constrain(kDif.green*nlDot, 0, 1), 255*constrain(kDif.blue*nlDot, 0, 1)));
        if (nlDot > 0)
        {
          PVector refDir = PVector.sub(PVector.mult(res_true.normal, 2*nlDot), ltg.direction);
